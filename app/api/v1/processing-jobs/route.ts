...
export const POST = withUser(async ({ req, user }) => {
  ...
  const priorPayload =
    existingQueue?.payload && typeof existingQueue.payload === "object"
      ...
      : {};
  const mergedQueuePayload: Record<string, unknown> = {
    ...priorPayload,
    ...queuePayload,
    bunnyUrls: {
      ...((priorPayload.bunnyUrls as Record<string, unknown> | undefined) ??
        {}),
      ...((queuePayload.bunnyUrls as Record<string, unknown> | undefined) ??
        {}),
    },
  };
  ...
  const [job] = await prisma.$transaction([
    ...
    prisma.multiplierQueueItem.upsert({
      where: { id: queueItemId },
      create: {
        id: queueItemId,
        userId: user.id,
  ...
  return json({ data: { job, queueItemId } }, { status: 201 });
});
