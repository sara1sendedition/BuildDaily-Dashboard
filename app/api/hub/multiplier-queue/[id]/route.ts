...
/** Same pattern as ../route.ts, for /api/v1/multiplier/queue/[id]. */
...
export async function GET(_req: NextRequest, ctx: ParamsCtx) {
  ...
  return forward("GET", `/api/v1/multiplier/queue/${encodeURIComponent(id)}`);
}
...
export async function PATCH(req: NextRequest, ctx: ParamsCtx) {
  ...
  return forward(
    "PATCH",
    `/api/v1/multiplier/queue/${encodeURIComponent(id)}`,
    body,
  );
}
