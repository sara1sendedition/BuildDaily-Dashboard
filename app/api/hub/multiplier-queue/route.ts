...
/**
 * Multiplier → Hub proxy for /api/v1/multiplier/queue (Phase 3).
 *
 * Same pattern as `app/api/hub/schedule/route.ts`: forwards GET/POST to the
 * Hub with a fresh Clerk JWT. Server-to-server avoids CORS.
 */
...
export async function GET(req: NextRequest) {
  ...
  return forward("GET", `/api/v1/multiplier/queue${qs}`);
}
...
export async function POST(req: NextRequest) {
  ...
  return forward("POST", "/api/v1/multiplier/queue", body);
}
